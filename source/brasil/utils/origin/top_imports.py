...
def top_importadores(dfs):
    ...
    for i in range(len(dfs)):
        ...
        for company in df['POSSIBLE CONSIGNEE'].unique():

            data = df[df['POSSIBLE CONSIGNEE'] == f"{company}"]

            if (data['Net weight'].sum() > 1500):

                volumenTotalCompania = (data['Net weight'].sum()/1000).round(2)
                ...
                company_dic['Proveedores'].append(
                    ', '.join(data['POSSIBLE SHIPPER'].unique().tolist()))
